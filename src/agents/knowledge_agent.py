        ...
        try:
            ...
            if action == "save":
                if not key or not content:
                    return TaskResult(
                        success=False, error_message="Key and content required for save"
                    )
                ...
                if not key:
                    return TaskResult(
                        success=False, error_message="Key required for retrieve"
                    )

                file_path = self.knowledge_dir / f"{key}.json"
                if not file_path.exists():
                    return TaskResult(
                        success=False, error_message=f"Knowledge not found: {key}"
                    )
