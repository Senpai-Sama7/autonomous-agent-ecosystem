...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        # Add context vars
        for name, var in [
            ("req_id", request_id_var),
            ("user_id", user_id_var),
            ("wf_id", workflow_id_var),
        ]:
            val = var.get()
            if val:
                log_data[name] = val
        ...
        # Add extra fields
        log_data.update(
            {k: v for k, v in record.__dict__.items() if k not in self.SKIP_FIELDS}
        )
...
class LogContext:
    ...
    def __init__(
        self, request_id: str = None, user_id: str = None, workflow_id: str = None
    ):
        self.vals = [
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug(
                ...
                extra={"duration_ms": (time.time() - start) * 1000, "status": "ok"},
            )
            return result
        except Exception as e:
            logger.error(
                f"{func.__name__} failed",
                extra={"duration_ms": (time.time() - start) * 1000, "error": str(e)},
                exc_info=True,
            )
            raise
